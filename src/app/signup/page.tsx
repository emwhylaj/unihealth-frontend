...
export default function SignUpPage() {
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      try {
        // Exchange the access_token for an id_token via userinfo, but the
        // backend expects an id_token — use "auth-code" flow to get id_token.
        // GoogleOAuthProvider implicit flow returns access_token; we need to
        // request id_token via the code flow instead.
  ...
  const handleAppleSignUp = async () => {
    ...
    if (!appleClientId || appleClientId === "com.unihealth.web") {
      toast.error(
        "Apple Sign In is not configured yet. Please use email sign-up."
      );
      return;
    }
    ...
    try {
      ...
      toast.error("Apple sign-up was cancelled or failed.");
    } finally {
        ...
        <div className="relative z-10 text-center">
          <div className="w-72 h-72 mx-auto mb-8 flex items-center justify-center">
            {role === "patient" ? (
              ...
              <svg viewBox="0 0 280 280" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
                ...
                <circle cx="140" cy="70" r="28" fill="#1a3a1a"/>
                <circle cx="130" cy="66" r="3" fill="#4ade80" opacity="0.8"/>
                <circle cx="150" cy="66" r="3" fill="#4ade80" opacity="0.8"/>
                <path d="M132 78 Q140 84 148 78" stroke="#4ade80" strokeWidth="2" strokeLinecap="round" opacity="0.8"/>
                {/* White coat body */}
                <path d="M100 180 Q108 140 140 132 Q172 140 180 180" fill="#1a3a1a"/>
                <path d="M108 135 L95 200" stroke="#2d5a2d" strokeWidth="18" strokeLinecap="round"/>
                <path d="M172 135 L185 200" stroke="#2d5a2d" strokeWidth="18" strokeLinecap="round"/>
                {/* Coat lapels */}
                <path d="M130 132 L118 160" stroke="#4ade80" strokeWidth="3" strokeLinecap="round" opacity="0.5"/>
                <path d="M150 132 L162 160" stroke="#4ade80" strokeWidth="3" strokeLinecap="round" opacity="0.5"/>
                {/* Stethoscope */}
                <path d="M155 150 Q175 155 178 172 Q181 188 168 192 Q155 196 152 183" stroke="#4ade80" strokeWidth="3" strokeLinecap="round" fill="none" opacity="0.9"/>
                <circle cx="152" cy="183" r="5" fill="#4ade80" opacity="0.9"/>
                {/* Clipboard */}
                <rect x="105" y="155" width="32" height="38" rx="4" fill="#0d2010" stroke="#4ade80" strokeWidth="1" opacity="0.8"/>
                <rect x="110" y="160" width="22" height="3" rx="1" fill="#4ade80" opacity="0.6"/>
                <rect x="110" y="167" width="18" height="3" rx="1" fill="#4ade80" opacity="0.4"/>
                <rect x="110" y="174" width="20" height="3" rx="1" fill="#4ade80" opacity="0.5"/>
                <rect x="116" y="150" width="10" height="6" rx="2" fill="#1a3a1a" stroke="#4ade80" strokeWidth="1"/>
                {/* Feet */}
                <ellipse cx="100" cy="206" rx="12" ry="8" fill="#2d4a2d"/>
                <ellipse cx="180" cy="206" rx="12" ry="8" fill="#2d4a2d"/>
              </svg>
            )}
          </div>
          ...
          <p className="text-gray-400 text-sm mt-6 max-w-xs">
            {role === "patient"
              ...
              : "Register your facility and manage patient records securely."}
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 lg:shadow-none lg:rounded-none">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {appleLoading ? "Signing up with Apple..." : "Continue with Apple"}
            </button>
            ...
            {/* Email */}
            <Link
              href={role === "hospital" ? "/signup/hospital/email" : "/signup/email"}
